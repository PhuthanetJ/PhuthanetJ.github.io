...
(function () {
    ...
    function parseConfig(doc) {
        ...
        const errors = validate(next.state, next.bindings); if (errors.length) throw Error(errors.join(' · ')); return next;
    }
    function apply(doc) { const next = parseConfig(doc); config.bindings = next.bindings; Object.assign(state, next.state); Object.assign(copy.th, next.copy.th); Object.assign(copy.en, next.copy.en); Object.assign(assets, next.assets); lists.wg = next.lists.wg; lists.mac = next.lists.mac; persist(); syncFields(); loadListeners.forEach(f => f()); }
    ...
    window.NT = { q, qa, on, esc, db, user, getUser, state, copy, assets, lists, currentSite, currentPortalId, bindings: () => config.bindings, setBindings, portalChoices, choosePortal, can, allowedIds, allowedSites: () => NT_DATA.sites.filter(s => allowedIds().includes(s.id)), siteName, fillSites, selectSite, changed, syncFields, toast, validate, download, snapshot, parseConfig, persist, storageStatus: () => ({ ...storageResult }), onChange: f => changeListeners.push(f), onLoad: f => loadListeners.push(f) };
