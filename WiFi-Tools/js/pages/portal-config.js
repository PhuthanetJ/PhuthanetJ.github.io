(function () {
    ...
    let acceptedTerms = '';
    const termsDialog = q('#nt-terms-dialog');
    const termsSignature = () => JSON.stringify([state.termsVersion, state.termsTh, state.termsEn, state.termsBodyTh, state.termsBodyEn]);
    function openTerms() {
        const en = language === 'en', body = en ? state.termsBodyEn : state.termsBodyTh;
        q('#nt-terms-title').textContent = en ? 'Free WiFi terms of use' : 'เงื่อนไขการใช้งาน Free WiFi';
        q('#nt-terms-version').textContent = (en ? 'Version: ' : 'รุ่นเงื่อนไข: ') + state.termsVersion;
        q('#nt-terms-body').textContent = body;
        q('#nt-terms-body').lang = language;
        q('#nt-terms-close').textContent = en ? 'Close' : 'ปิด';
        q('#nt-terms-confirm').textContent = en ? 'Accept terms' : 'ยอมรับเงื่อนไข';
        q('#nt-terms-confirm').disabled = !body.trim();
        termsDialog.showModal();
        q('#nt-terms-body').scrollTop = 0;
    }
    q('#nt-terms-close').addEventListener('click', () => termsDialog.close());
    q('#nt-terms-confirm').addEventListener('click', () => {
        if (!state.termsEnabled || !(language === 'en' ? state.termsBodyEn : state.termsBodyTh).trim()) return;
        acceptedTerms = termsSignature();
        q('#nt-public-accept').checked = true;
        termsDialog.close();
        feedback(language === 'en' ? 'Terms accepted. You can request free WiFi.' : 'ยอมรับเงื่อนไขแล้ว สามารถกดรับสิทธิ์ใช้งานฟรีได้', true);
    });
    ...
    function preview() {
        ...
        if (state.termsEnabled) html += '<div class="nt-public-terms"><input type="checkbox" id="nt-public-accept" aria-labelledby="nt-public-terms-link"' + (acceptedTerms === termsSignature() ? ' checked' : '') + '><button type="button" id="nt-public-terms-link" class="wt-terms-link" aria-haspopup="dialog" aria-controls="nt-terms-dialog">' + esc(en ? state.termsEn : state.termsTh) + '</button></div>';
        ...
        q('#nt-public-accept')?.addEventListener('click', event => {
            if (event.currentTarget.checked) { event.preventDefault(); openTerms(); }
            else acceptedTerms = '';
        });
