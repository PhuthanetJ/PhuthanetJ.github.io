/* Page: portal-config — local prototype; no backend calls. */
(function () {
  'use strict';
 ...
 function preview(){
  ...
  if(q('#nt-public-connect'))q('#nt-public-connect').addEventListener('click',()=>{
   ...
   if(state.guest&&state.voucherMode==='coupon'&&!/^DEMO-0[1-9]$|^DEMO-10$/.test(q('#nt-public-coupon').value.trim()))return feedback(en?'Use a sample code, such as DEMO-01.':'ใช้รหัสตัวอย่าง เช่น DEMO-01',false);
 ...
 NT.onChange(() => {previewMode='normal'; preview();});
 NT.onLoad(() => {
   previewMode='normal'; proposal=null; q('#nt-ai-review').hidden=true;
   qa('[data-asset]').forEach(el=>el.value=''); fillCopy(); preview();
 });
 fillCopy(); preview(); setDevice('desktop');
