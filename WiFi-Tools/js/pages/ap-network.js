(function () {
    ...
    function listTable(type) {
        ...
        host.querySelectorAll('[data-remove]').forEach(el => el.disabled = !NT.can('editSite')); host.querySelectorAll('[data-remove]').forEach(el => el.addEventListener('click', () => { if (!NT.can('editSite')) return; arr.splice(Number(el.dataset.index), 1); changed(); listTable(type); toast('ลบรายการออกจาก Draft แล้ว'); }));
    }
    ...
    listTable('wg'); listTable('mac'); NT.onLoad(() => { listTable('wg'); listTable('mac'); });
})();