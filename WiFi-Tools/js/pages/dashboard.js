/* Page: dashboard — local prototype; no backend calls. */
(function () {
  'use strict';
  const {q, qa, esc, state, copy, assets, lists, changed, syncFields, toast, validate} = window.NT;
 const dashSites=NT_DATA.dashboard.sites, dashIncidents=NT_DATA.dashboard.incidents;
 ...
 function drawDashboard(){
  ...
  const times=NT_DATA.dashboard.times;
  ...
  q('#nt-dash-sites').innerHTML=sites.map(s=>'<tr><td>'+s.name+'</td><td>'+s.active+'</td><td>'+s.online+' / '+s.aps+'</td><td>'+(100*s.success/s.attempts).toFixed(1)+'%<br><span class="nt-help">'+s.success+' / '+s.attempts+'</span></td><td>'+s.traffic.toFixed(1)+' GB</td><td><span class="nt-badge '+(s.alerts?'pending':'good')+'">'+(s.alerts?s.alerts+' เหตุการณ์':'ไม่มี')+'</span></td></tr>').join('');
  const methods=NT_DATA.dashboard.methods;
