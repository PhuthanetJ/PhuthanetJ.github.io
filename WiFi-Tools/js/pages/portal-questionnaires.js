(function () {
    ...
    function load(id) {
        ...
        q('#wt-question-name').value = item?.name || ''; q('#wt-question-type').value = item?.type || 'survey'; q('#wt-question-text').value = item?.question || '';
    ...
    function renderChoices() {
        q('#wt-portal-question-choices').innerHTML = lists.questionnaires.length ? lists.questionnaires.map(item => '<label class="nt-check"><input type="checkbox" data-portal-question="' + item.id + '"' + (lists.portalQuestionnaireIds.includes(item.id) ? ' checked' : '') + (can('editSite') ? '' : ' disabled') + '><span>' + esc(item.name) + ' · ' + esc(item.question) + '</span></label>').join('') : '<p>ยังไม่มีคำถาม สร้างได้ในฟอร์มด้านล่าง</p>';
    }
    function render(id = editingId) {
        q('#wt-question-select').innerHTML = '<option value="">สร้างคำถามใหม่</option>' + lists.questionnaires.map(item => '<option value="' + item.id + '">' + esc(item.name) + '</option>').join('');
    ...
    q('#wt-question-save').addEventListener('click', () => {
        ...
        try {
            ...
            const item = { id, name: q('#wt-question-name').value, type: q('#wt-question-type').value, question: q('#wt-question-text').value, answers: answers(), correctAnswer: q('#wt-question-correct').value };
    ...
    function selectedQuestions() { return lists.portalQuestionnaireIds.map(id => lists.questionnaires.find(item => item.id === id)).filter(Boolean); }
    function portalMarkup(language) {
        const questions = selectedQuestions();
        if (!questions.length) return '<p class="nt-public-message">ยังไม่ได้เลือก Questionnaire สำหรับ Portal</p>';
        return questions.map(item => '<label class="wt-public-question">' + esc(item.question) + '<select id="wt-public-question-' + item.id + '" class="nt-public-input"><option value="">' + (language === 'en' ? 'Select an answer' : 'เลือกคำตอบ') + '</option>' + item.answers.map((answer, i) => '<option value="' + i + '">' + esc(answer) + '</option>').join('') + '</select></label>').join('');
    }
    function gradePortal() {
        const questions = selectedQuestions(), answers = Object.create(null);
        if (!questions.length || questions.length !== lists.portalQuestionnaireIds.length) return { ok: false, missing: [], incorrect: [] };
        for (const item of questions) { const input = q('#wt-public-question-' + item.id); if (input && input.value !== '') answers[item.id] = item.answers[Number(input.value)]; }
